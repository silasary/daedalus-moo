...
namespace Chiroptera.Win.MCP.Packages
{
    ...
    {
        ListView UserList;
        MOO.MOOObject you = new Chiroptera.Win.MOO.MOOObject("#-1");
        string[] Fields;
        ...
        public void HandleMessage(string command, Dictionary<string, string> KeyVals)
        {
            if (command == "dns-com-vmoo-userlist")
            {
                ...
                if (!MCPHandler.ContainsKeys(KeyVals,"_data-tag"))
                    return;
                MCPHandler.RegisterMultilineHandler(KeyVals["_data-tag"], "dns-com-vmoo-userlist-multiline");
            }
            else if (command == "dns-com-vmoo-userlist-you")
            {
                if (!MCPHandler.ContainsKeys(KeyVals, "nr"))
                    return;
                you = new Chiroptera.Win.MOO.MOOObject(KeyVals["nr"]);
            }
        ...
        public void Negotiated()
        {
            ...
            ClientCore.s_clientCore.MainWindow.Controls.Add(UserList);
        }
