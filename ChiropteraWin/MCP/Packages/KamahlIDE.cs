...
namespace Chiroptera.Win.MCP.Packages
{
    ...
    {
        public static KamahlIDE _idepackage;
        public KamahlIDE()
        {
            _idepackage = this;
        }
        ...
        public void HandleMessage(string command, Dictionary<string, string> KV)
        {
            ...
            else if (command == "dns-silasary-IDE-details")
            {
                ...
                {
                    MCPHandler.RegisterMultilineHandler(KV["_data-tag"], "dns-silasary-IDE-details-" + KV["object"].ToString());
                    return;
                }
        ...
        internal void RequestParent(Chiroptera.Win.MOO.MOOObject mOOObject)
        {
            if (requestedParents.Contains(mOOObject))
                return;
            MCPHandler._mcpHandler.SendOOB("dns-silasary-IDE-parent", MCPHandler.CreateKeyvals("object", mOOObject.ToString()));
            requestedParents.Add(mOOObject);
        }
        List<MOOObject> requestedDetails = new List<MOOObject>();
        internal void RequestDetails(Chiroptera.Win.MOO.MOOObject mOOObject)
        {
            if (requestedDetails.Contains(mOOObject))
                return;
            MCPHandler._mcpHandler.SendOOB("dns-silasary-IDE-details", MCPHandler.CreateKeyvals("object", mOOObject.ToString()));
            requestedDetails.Add(mOOObject);
        }

        internal void RequestProperty(Chiroptera.Win.MOO.MOOObject mOOObject, string propname)
        {
            MCPHandler._mcpHandler.SendOOB("dns-silasary-IDE-property", MCPHandler.CreateKeyvals("object", mOOObject.ToString(), "property", propname));
        }
    }
}
